...
- 单引号中$varaible的形式会被当做普通文本，而双引号会把它当做变量看待
...
declare使用时若不接任何参数，bash会输出所有的变量名称和内容，等同于set。
...
注意：变量默认类型是字符串；bash环境的运算只能是整型，1/2 = 0。typeset用法同declare。
...
bash首先会去读系统的配置文件：/etc/profile。读取完毕后再去读用户的配置文件，按照以下顺序：`~/.bash_profile`、`~/.bash_login`、`~/.profile`。读取到一个文件后即停止读取。
...
```bash
ll /home > test     #内容写入到test文件中
find / > result 2> err    #将结果写入result，错误写入err
...
```bash
echo $PATH | cut -d ":" -f 3, 5    # 取出PATH中第三和第五段内容，以：作为分隔符
echo $PATH | cut -c 3-5           # 取出PATH中第3-5个字符（无卵用，懒得找例子）
```
