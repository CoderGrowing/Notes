...
Node 就是存储数据的键值对，它包含了四个字段，具体的实现如下：
...
1. 扩容：创建一个新的 Entry 数组，大小为原来的 2 倍（JDK 1.7 源码）
...
2. ReHash，因为数组大小不同，Hash 的规则也不同了，所以需要进行重新 Hash
...
多线程下 HashMap 会有线程安全的问题，主要是因为 HashMap 需要进行 resize 的操作。Map 进行 put 操作时，如果同时出发了 rehash 操作，会导致 HashMap 中可能出现循环节点。参考酷壳的文章 [疫苗：JAVA HASHMAP 的死循环 ](https://coolshell.cn/articles/9606.html)。
