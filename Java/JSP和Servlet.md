...
容器得到对 Servlet 的请求时，会创建 HttpServletResponse 和 HttpServletRequest 对象，分别代表请求和响应。并将这两个对象传递个 Servlet。HttpServletResponse 和 HttpServletRequest 是两个接口，由容器来实现。
...
cookies，发送 cookie/ 从请求得到会话 ID 都由容器进行，只需要
